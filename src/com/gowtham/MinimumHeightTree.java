...
public class MinimumHeightTree {
	 ...
	 * 
	 * (3) The degree of a vertex of a graph is the number of edges incident to
	 * the vertex.
	 * 
	 * (4) A leaf is a vertex of degree 1. An internal vertex is a vertex of
	 * degree at least 2.
	 * 
	 * (5) A path graph is a tree with two or more vertices that is not branched
	 * at all.
	 * 
	 * (6) A tree is called a rooted tree if one vertex has been designated the
	 * root.
	 * 
	 ...
	 * we mean vertex of degree 1 (aka leaves). We let the pointers move the
	 * same speed. When two pointers meet, we keep only one of them, until the
	 * last two pointers meet or one step away we then find the roots.
	 * 
	 * It is easy to see that the last two pointers are from the two ends of the
	 * longest path in the graph.
	 * 
	 * The actual implementation is similar to the BFS topological sort. Remove
	 * the leaves, update the degrees of inner vertexes. Then remove the new
	 * leaves. Doing so level by level until there are 2 or 1 nodes left. What's
	 * left is our answer!
	 * 
